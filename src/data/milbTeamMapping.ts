// MiLB Team ID Mapping
// Maps venue IDs to team IDs for fetching schedules from MLB Stats API
// Generated from MLB Stats API on 2025

export const MILB_VENUE_TO_TEAM_MAP: Record<number, number> = {
  // AAA Teams
  2542: 235,  // AutoZone Park -> Memphis Redbirds
  2745: 529,  // Cheney Stadium -> Tacoma Rainiers
  2843: 238,  // Chickasaw Bricktown Ballpark -> Oklahoma City Comets
  5472: 1960,  // CHS Field -> St. Paul Saints
  3230: 1410,  // Coca-Cola Park -> Lehigh Valley IronPigs
  5421: 5434,  // Constellation Field -> Sugar Land Space Cowboys
  3810: 431,  // Coolray Field -> Gwinnett Stripers
  2528: 102,  // Dell Diamond -> Round Rock Express
  2541: 234,  // Durham Bulls Athletic Park -> Durham Bulls
  2767: 512,  // Fifth Third Field -> Toledo Mud Hens
  4715: 556,  // First Horizon Park -> Nashville Sounds
  3789: 2310,  // Greater Nevada Field -> Reno Aces
  2781: 568,  // Harbor Park -> Norfolk Tides
  3970: 445,  // Huntington Park -> Columbus Clippers
  2773: 534,  // Innovative Field -> Rochester Red Wings
  2683: 342,  // Isotopes Park -> Albuquerque Isotopes
  5355: 400,  // Las Vegas Ballpark -> Las Vegas Aviators
  2724: 416,  // Louisville Slugger Field -> Louisville Bats
  2823: 552,  // NBT Bank Stadium -> Syracuse Mets
  2797: 531,  // PNC Field -> Scranton/Wilkes-Barre RailRiders
  5410: 533,  // Polar Park -> Worcester Red Sox
  2844: 451,  // Principal Park -> Iowa Cubs
  2756: 422,  // Sahlen Field -> Buffalo Bisons
  4669: 4904,  // Southwest University Park -> El Paso Chihuahuas
  2529: 105,  // Sutter Health Park -> Sacramento River Cats
  6135: 561,  // The Ballpark at America First Square -> Salt Lake Bees
  4670: 494,  // Truist Field -> Charlotte Knights
  2858: 484,  // Victory Field -> Indianapolis Indians
  2852: 564,  // Vystar Ballpark -> Jacksonville Jumbo Shrimp
  4271: 541,  // Werner Park -> Omaha Storm Chasers
  
  // AA Teams
  3329: 1350,  // Arvest Ballpark -> Northwest Arkansas Naturals
  2682: 498,  // AT&T Field -> Chattanooga Lookouts
  4329: 4124,  // Blue Wahoos Stadium -> Pensacola Blue Wahoos
  2740: 402,  // Canal Park -> Akron RubberDucks
  6136: 553,  // Covenant Health Park -> Knoxville Smokies
  2779: 546,  // Delta Dental Park -> Portland Sea Dogs
  2868: 463,  // Delta Dental Stadium -> New Hampshire Fisher Cats
  3389: 574,  // Dickey-Stephens Park -> Arkansas Travelers
  4985: 538,  // Dunkin' Park -> Hartford Yard Goats
  5450: 3898,  // Equity Bank Park -> Wichita Wind Surge
  2769: 522,  // FirstEnergy Stadium -> Reading Fightin Phils
  2749: 547,  // FNB Field -> Harrisburg Senators
  2722: 440,  // Hammons Field -> Springfield Cardinals
  5415: 5368,  // Hodgetown -> Amarillo Sod Poodles
  4960: 5015,  // Keesler Federal Park -> Biloxi Shuckers
  2820: 505,  // Mirabito Stadium -> Binghamton Rumble Ponies
  2768: 237,  // Momentum Bank Ballpark -> Midland RockHounds
  2814: 421,  // Montgomery Riverwalk Stadium -> Montgomery Biscuits
  2818: 510,  // Nelson Wolff Stadium -> San Antonio Missions
  4149: 260,  // ONEOK Field -> Tulsa Drillers
  2733: 452,  // Peoples Natural Gas Field -> Altoona Curve
  2832: 418,  // Prince George's Stadium -> Chesapeake Baysox
  4529: 247,  // Regions Field -> Birmingham Barons
  2755: 540,  // Riders Field -> Frisco RoughRiders
  2776: 6325,  // Synovus Park -> Columbus Clingstones
  5418: 1956,  // TD Bank Ballpark -> Somerset Patriots
  2853: 3410,  // The Diamond -> Richmond Flying Squirrels
  5455: 559,  // Toyota Field -> Rocket City Trash Pandas
  2512: 106,  // UPMC Park -> Erie SeaWolves
  2861: 482,  // Whataburger Field -> Corpus Christi Hooks
  
  // A+ Teams
  5525: 554,  // ABC Supply Stadium -> Beloit Sky Carp
  2851: 432,  // AdventHealth Stadium -> Rome Emperors
  2730: 486,  // Avista Stadium -> Spokane Indians
  3569: 2498,  // Bowling Green Ballpark -> Bowling Green Hot Rods
  2759: 437,  // Classic Auto Group Park -> Lake County Captains
  2766: 459,  // Day Air Ballpark -> Dayton Dragons
  3189: 456,  // Dow Diamond -> Great Lakes Loons
  2821: 443,  // Dozer Park -> Peoria Chiefs
  6134: 6324,  // Fifth Third Park -> Hub City Spartanburgers
  2723: 477,  // First National Bank Field -> Greensboro Grasshoppers
  3429: 428,  // Fluor Field at the West End -> Greenville Drive
  2850: 550,  // Four Winds Field -> South Bend Cubs
  2793: 426,  // Frawley Stadium -> Wilmington Blue Rocks
  2762: 403,  // Funko Field -> Everett AquaSox
  2826: 460,  // Gesa Stadium -> Tri-City Dust Devils
  2757: 537,  // Heritage Financial Park -> Hudson Valley Renegades
  4429: 419,  // Hillsboro Ballpark -> Hillsboro Hops
  2822: 499,  // Jackson Field -> Lansing Lugnuts
  2765: 582,  // LMCU Ballpark -> West Michigan Whitecaps
  2795: 453,  // Maimonides Park -> Brooklyn Cyclones
  2807: 573,  // McCormick Field -> Asheville Tourists
  2790: 565,  // Modern Woodmen Park -> Quad Cities River Bandits
  2771: 572,  // Neuroscience Group Field -> Wisconsin Timber Rattlers
  3790: 584,  // Parkview Field -> Fort Wayne TinCaps
  4109: 461,  // PK Park -> Eugene Emeralds
  2837: 488,  // Ripken Stadium -> Aberdeen IronBirds
  2817: 435,  // Rogers Field at Nat Bailey Stadium -> Vancouver Canadians
  2866: 427,  // ShoreTown Ballpark -> Jersey Shore BlueClaws
  4089: 580,  // Truist Stadium -> Winston-Salem Dash
  2857: 492,  // Veterans Memorial Stadium -> Cedar Rapids Kernels
  
  // A Teams
  2729: 548,  // Arthur W. Perdue Stadium -> Delmarva Shorebirds
  2764: 487,  // Atrium Health Ballpark -> Kannapolis Cannon Ballers
  2803: 481,  // Bank of the James Stadium -> Lynchburg Hillcats
  2731: 524,  // Banner Island Ballpark -> Stockton Ports
  2700: 566,  // BayCare Ballpark -> Clearwater Threshers
  2840: 414,  // Carilion Clinic Field -> Salem Red Sox
  2640: 259,  // Chukchansi Park -> Fresno Grizzlies
  2856: 507,  // Clover Park -> St. Lucie Mets
  2815: 476,  // Excite Ballpark -> San Jose Giants
  2625: 249,  // Five County Stadium (former) -> Wilson Warbirds
  7254: 249,  // Wilson Ballpark -> Wilson Warbirds
  2787: 450,  // Jackie Robinson Ballpark -> Daytona Tortugas
  2791: 515,  // John Thurman Field -> Modesto Nuts
  2540: 233,  // Joseph P. Riley, Jr. Ballpark -> Charleston RiverDogs
  2796: 448,  // L.P. Frans Stadium -> Hickory Crawdads
  2526: 3390,  // LECOM Park -> Bradenton Marauders
  2862: 509,  // Lee Health Sports Complex -> Fort Myers Mighty Mussels
  2854: 526,  // LoanMart Field -> Rancho Cucamonga Quakes
  2747: 521,  // Pelicans Ballpark -> Myrtle Beach Pelicans
  2511: 570,  // Publix Field at Joker Marchant Stadium -> Lakeland Flying Tigers
  2520: 479,  // Roger Dean Chevrolet Stadium -> Jupiter Hammerheads
  2728: 401,  // San Manuel Stadium -> Inland Empire 66ers
  4980: 3705,  // Segra Park -> Columbia Fireflies
  5400: 3712,  // SEGRA Stadium -> Fayetteville Woodpeckers
  5345: 478,  // SRP Park -> Augusta GreenJackets
  2536: 424,  // TD Ballpark -> Dunedin Blue Jays
  2516: 103,  // The Diamond -> Lake Elsinore Storm
  2835: 516,  // Valley Strong Ballpark -> Visalia Rawhide
  5520: 436,  // Virginia Credit Union Stadium -> Fredericksburg Nationals
  7250: 587,  // Yankee Complex Field 2 -> Tampa Tarpons
};

// Helper function to get team ID from venue ID
export function getTeamIdFromVenueId(venueId: number): number | null {
  return MILB_VENUE_TO_TEAM_MAP[venueId] || null;
}

// Reverse mapping: Team ID to Venue ID
export const MILB_TEAM_TO_VENUE_MAP: Record<number, number> = {};
Object.entries(MILB_VENUE_TO_TEAM_MAP).forEach(([venueId, teamId]) => {
  MILB_TEAM_TO_VENUE_MAP[teamId] = parseInt(venueId);
});

// String ID to Venue ID mapping for MiLB stadiums
export const MILB_STRING_ID_TO_VENUE_ID_MAP: Record<string, number> = {
  // AAA Teams
  'buffalo-bisons': 2756,
  'charlotte-knights': 4670,
  'columbus-clippers': 3970,
  'durham-bulls': 2541,
  'gwinnett-stripers': 3810,
  'indianapolis-indians': 2858,
  'iowa-cubs': 2844,
  'jacksonville-jumbo-shrimp': 2852,
  'lehigh-valley-ironpigs': 3230,
  'louisville-bats': 2724,
  'memphis-redbirds': 2542,
  'nashville-sounds': 4715,
  'norfolk-tides': 2781,
  'omaha-storm-chasers': 4271,
  'rochester-red-wings': 2773,
  'scranton-railriders': 2797,
  'st-paul-saints': 5472,
  'sugar-land-space-cowboys': 5421,
  'syracuse-mets': 2823,
  'tacoma-rainiers': 2745,
  'toledo-mud-hens': 2767,
  'worcester-red-sox': 5410,
  'albuquerque-isotopes': 2683,
  'el-paso-chihuahuas': 4669,
  'las-vegas-aviators': 5355,
  'oklahoma-city-dodgers': 2843,
  'reno-aces': 3789,
  'round-rock-express': 2528,
  'sacramento-river-cats': 2529,
  'salt-lake-bees': 6135,
  
  // AA Teams
  'akron-rubberducks': 2740,
  'altoona-curve': 2733,
  'biloxi-shuckers': 4960,
  'binghamton-rumble-ponies': 2820,
  'birmingham-barons': 4529,
  'bowie-baysox': 2832,
  'chattanooga-lookouts': 2682,
  'corpus-christi-hooks': 2861,
  'erie-seawolves': 2512,
  'frisco-roughriders': 2755,
  'harrisburg-senators': 2749,
  'hartford-yard-goats': 4985,
  'midland-rockhounds': 2768,
  'montgomery-biscuits': 2814,
  'new-hampshire-fisher-cats': 2868,
  'northwest-arkansas-naturals': 3329,
  'pensacola-blue-wahoos': 4329,
  'portland-sea-dogs': 2779,
  'reading-fightin-phils': 2769,
  'richmond-flying-squirrels': 2853,
  'rocket-city-trash-pandas': 5455,
  'san-antonio-missions': 2818,
  'springfield-cardinals': 2722,
  'tulsa-drillers': 4149,
  'wichita-wind-surge': 5450,
  'somerset-patriots': 5418,
  'arkansas-travelers': 3389,
  'amarillo-sod-poodles': 5415,
  'knoxville-smokies': 6136,
  'columbus-clingstones': 2776,
  
  // A+ Teams
  'aberdeen-ironbirds': 2837,
  'asheville-tourists': 2807,
  'beloit-sky-carp': 5525,
  'bowling-green-hot-rods': 3569,
  'brooklyn-cyclones': 2795,
  'cedar-rapids-kernels': 2857,
  'dayton-dragons': 2766,
  'everett-aquasox': 2762,
  'fort-wayne-tincaps': 3790,
  'great-lakes-loons': 3189,
  'greensboro-grasshoppers': 2723,
  'greenville-drive': 3429,
  'hillsboro-hops': 4429,
  'hudson-valley-renegades': 2757,
  'jersey-shore-blueclaws': 2866,
  'lake-county-captains': 2759,
  'lansing-lugnuts': 2822,
  'peoria-chiefs': 2821,
  'quad-cities-river-bandits': 2790,
  'rome-braves': 2851,
  'south-bend-cubs': 2850,
  'spokane-indians': 2730,
  'tri-city-dust-devils': 2826,
  'vancouver-canadians': 2817,
  'west-michigan-whitecaps': 2765,
  'wilmington-blue-rocks': 2793,
  'winston-salem-dash': 4089,
  'wisconsin-timber-rattlers': 2771,
  'eugene-emeralds': 4109,
  'hub-city-spartanburgers': 6134,
  
  // A Teams
  'augusta-greenjackets': 5345,
  'bradenton-marauders': 2526,
  'carolina-mudcats': 7254,
  'down-east-wood-ducks': 6134,
  'charleston-riverdogs': 2540,
  'clearwater-threshers': 2700,
  'columbia-fireflies': 4980,
  'daytona-tortugas': 2787,
  'delmarva-shorebirds': 2729,
  'dunedin-blue-jays': 2536,
  'fayetteville-woodpeckers': 5400,
  'fort-myers-mighty-mussels': 2862,
  'fredericksburg-nationals': 5520,
  'fresno-grizzlies': 2640,
  'hickory-crawdads': 2796,
  'inland-empire-66ers': 2728,
  'jupiter-hammerheads': 2520,
  'kannapolis-cannon-ballers': 2764,
  'lake-elsinore-storm': 2516,
  'lakeland-flying-tigers': 2511,
  'lynchburg-hillcats': 2803,
  'modesto-nuts': 2791,
  'myrtle-beach-pelicans': 2747,
  'rancho-cucamonga-quakes': 2854,
  'salem-red-sox': 2840,
  'san-jose-giants': 2815,
  'st-lucie-mets': 2856,
  'stockton-ports': 2731,
  'tampa-tarpons': 7250,
  'visalia-rawhide': 2835,
};

// Helper function to get venue ID from string ID
export function getVenueIdFromStringId(stringId: string): number | null {
  return MILB_STRING_ID_TO_VENUE_ID_MAP[stringId] || null;
}