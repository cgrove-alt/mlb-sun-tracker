...
export class ShadeValidationSystem {
  ...
  public runValidation(): ValidationMetrics {
    ...
    errorsByTime.forEach((errors, time) => {
    ...
    errorsByStadium.forEach((errors, stadium) => {
  ...
  public generateReport(): string {
    ...
    metrics.accuracyByTimeOfDay.forEach((accuracy, time) => {
      report += `- ${time}: ${accuracy.toFixed(2)}%\n`;
    });
    report += '\n';
    
    report += `## Accuracy by Stadium\n`;
    metrics.accuracyByStadium.forEach((accuracy, stadium) => {
