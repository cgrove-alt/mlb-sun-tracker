...
function UnifiedAppContentOptimized() {
  ...
  const handleVenueSelect = useCallback((venue: UnifiedVenue) => {
  ...
  useEffect(() => {
    const calculateShade = async () => {
      ...
      try {
        const shadeData = await getUnifiedVenueShade(
          selectedVenue,
          gameDateTime,
          weatherForecast || undefined
        );
        
        setShadedSections(shadeData.sections);
        setSunPosition(shadeData.sunPosition);
        
        // Convert to legacy format for compatibility
        const legacySections = shadeData.sections.map(section => ({
          ...section,
          shadePercentage: section.shadeInfo?.percentageInShade || 0,
          sunExposure: section.shadeInfo?.sunExposure || 'Unknown',
        ...
        <main className="App-main">
          ...
          <div className="controls">
            <UnifiedGameSelector
              ...
              stadiumGames={stadiumGames}
              setStadiumGames={setStadiumGames}
            />
          </div>
