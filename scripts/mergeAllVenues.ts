...
// Read MiLB venues from the json file (if it exists)
let milbVenues: any[] = [];
const milbPath = path.join(__dirname, '../milb-venues.json');
if (fs.existsSync(milbPath)) {
  const milbData = JSON.parse(fs.readFileSync(milbPath, 'utf-8'));
  milbVenues = milbData.venues || [];
}
