#!/usr/bin/env python3
"""Propagate uncertainty after adding the calibration-only far-field control."""
...
ANALYSIS_VERSION = "rockies-panorama-field-grade-left-field-calibration-pose-uncertainty-v1"
...
def main() -> None:
    ...
    parser.add_argument("calibration_pose", type=Path)
    ...
    pose_bytes = args.calibration_pose.read_bytes()
    pose = json.loads(pose_bytes)
    if pose.get("analysisVersion") != "rockies-panorama-field-grade-left-field-calibration-pose-v1":
        raise ValueError("Unsupported far-field calibration pose")
    if not pose["geometryBoundary"].get("establishesCandidateHistoricalFieldGradeCalibrationPose"):
        raise ValueError("Far-field calibration pose did not pass candidate gates")
    if not pose["partitionIntegrity"].get("leftFieldFeatureCalibrationOnly"):
        raise ValueError("Far-field feature partition is not calibration only")
    if pose["partitionIntegrity"].get("leftFieldFeatureEligibleForFutureHoldout"):
        raise ValueError("Far-field calibration feature cannot be a holdout")
    ...
    nominal_pixels = np.asarray(
        [item["observedPixel"] for item in controls], dtype=np.float64
    )
    pixel_uncertainties_95 = np.asarray(
        [item["pixelUncertainty95"] for item in controls], dtype=np.float64
    )
    pixel_sigmas = pixel_uncertainties_95 / RADIAL_NORMAL_95_TO_SIGMA
    angular_sigmas = pixel_sigmas / (size / 2.0)
    horizontal_uncertainties_95 = np.asarray(
        [item["horizontalWorldUncertainty95Feet"] for item in controls],
        dtype=np.float64,
    )
    horizontal_sigmas = horizontal_uncertainties_95 / RADIAL_NORMAL_95_TO_SIGMA
    vertical_uncertainties_95 = np.asarray(
        [item["relativeElevationUncertainty95Feet"] for item in controls],
        dtype=np.float64,
    )
    ...
    field_indices = np.asarray(
        [index for index, item in enumerate(controls) if item["role"] == "regulation-field-calibration"],
        dtype=np.int64,
    )
    if len(field_indices) != 4:
        raise ValueError("Exactly four infield controls are required for the grade plane")
    ...
    mound_center_uncertainty_95 = float(np.max(horizontal_uncertainties_95[field_indices]))
    mound_center_sigma = mound_center_uncertainty_95 / RADIAL_NORMAL_95_TO_SIGMA
    ...
    for index in range(args.samples):
        ...
        sampled_worlds[:, :2] += rng.normal(
            0.0, horizontal_sigmas[:, None], size=(len(sampled_worlds), 2)
        )
        sampled_worlds[:, 2] += rng.normal(0.0, vertical_sigmas)
        sampled_plane = fit_plane(
            sampled_worlds[field_indices, :2], sampled_worlds[field_indices, 2]
        )
        sampled_mound_center = nominal_mound_center + rng.normal(
            0.0, mound_center_sigma, size=2
        )
        ...
        if not np.all(np.isfinite(fitted.x)) or not np.all(np.isfinite(fitted.fun)):
            raise ValueError("Monte Carlo far-field fit produced non-finite values")
        if not fitted.success and np.max(np.abs(fitted.fun)) > 10.0:
            raise ValueError("Monte Carlo far-field fit did not converge")
        camera = fitted.x[:3]
    ...
    stable = {
        ...
        "sectionId": "207",
        "inputs": {
            "calibrationPosePath": str(args.calibration_pose),
            "calibrationPoseSha256": hashlib.sha256(pose_bytes).hexdigest(),
            "calibrationPoseArtifactVersion": pose["artifactVersion"],
        },
        "partitionIntegrity": pose["partitionIntegrity"],
        "selectedPose": selected,
        "uncertaintyInputs": {
            "pointPixelRadialUncertainties95": {
                item["controlId"]: item["pixelUncertainty95"] for item in controls
            },
            "pointWorldHorizontalRadialUncertainties95Feet": {
                item["controlId"]: item["horizontalWorldUncertainty95Feet"]
        ...
        "releaseGates": {
            ...
            "independentHoldoutGatePassed": False,
        },
    }
    blockers = []
    ...
    artifact = {
        "schemaVersion": 1,
        "artifactKind": "rockies-panorama-field-grade-left-field-calibration-pose-uncertainty-audit",
        "artifactStage": "far-field-calibration-uncertainty-propagated-independent-holdout-pending",
        "artifactVersion": artifact_version(stable),
        **stable,
        "geometryBoundary": {
            "establishesReleaseCameraPose": False,
            "note": "The far-field calibration feature is propagated as training evidence only. Currentness, mound survey, and a new independent holdout remain required.",
        },
