#!/usr/bin/env python3
"""Refit the full Section 207 pose with all passed regulation field evidence."""
...
ANALYSIS_VERSION = "rockies-panorama-all-regulation-field-pose-v1"


def circle_tangent_world_points(
    camera_xy: np.ndarray,
    center_xy: np.ndarray,
    radius: float,
) -> np.ndarray | None:
    center_to_camera = camera_xy - center_xy
    distance_squared = float(np.dot(center_to_camera, center_to_camera))
    if distance_squared <= radius * radius:
        return None
    base = center_xy + radius * radius / distance_squared * center_to_camera
    perpendicular = np.asarray(
        [-center_to_camera[1], center_to_camera[0]], dtype=np.float64
    )
    scale = radius * math.sqrt(distance_squared - radius * radius) / distance_squared
    return np.vstack((base + scale * perpendicular, base - scale * perpendicular))


def predicted_circle_tangent_pixels(
    ...
    for tangent in tangents:
        world = np.asarray([tangent[0], tangent[1], 0.0])
        direction = world - camera
        direction /= np.linalg.norm(direction)
        provider = rotate.T @ direction
...
def all_field_residuals(
    ...
    tangents = predicted_circle_tangent_pixels(
        parameters, mound_center, mound_radius, face, size
    )
    if tangents is None:
...
def main() -> None:
    ...
    parser.add_argument("full_pose", type=Path)
    parser.add_argument("output", type=Path)
    args = parser.parse_args()
    full_bytes = args.full_pose.read_bytes()
    full = json.loads(full_bytes)
    if full.get("analysisVersion") != "rockies-panorama-full-orientation-field-pose-v1":
        raise ValueError("Unsupported full-orientation pose")
    if not full["geometryBoundary"].get("moundCirclePrefitHoldoutPassed"):
        raise ValueError("Mound circle did not pass before promotion to calibration")
    ...
    selected = full["selectedSolution"]
    ...
    for (face, pixel, uncertainty, identifier), world in zip(controls, world_points):
        ...
        predicted_face, predicted_x, predicted_y = provider_ray_to_pixel(provider, size)
    ...
    fitted_tangents = predicted_circle_tangent_pixels(
        fitted.x, mound_center, mound_radius, "l", size
    )
    if fitted_tangents is None:
        raise ValueError("All-field pose does not project mound tangencies on the left face")
    tangent_records = {
        ...
        "residualPixels": np.abs(
            np.asarray(fitted_tangents) - observed_tangents
        ).tolist(),
    }
    stable = {
        ...
        "sectionId": "207",
        "inputs": {
            "fullOrientationPosePath": str(args.full_pose),
            "fullOrientationPoseSha256": hashlib.sha256(full_bytes).hexdigest(),
            "fullOrientationPoseArtifactVersion": full["artifactVersion"],
            "fieldRegistrationArtifactVersion": registration["artifactVersion"],
        },
        "calibrationPromotion": {
            "moundCirclePassedIndependentPrefitHoldoutBeforePromotion": True,
            "priorHoldoutArtifactVersion": full["artifactVersion"],
            "newIndependentValidationRequired": True,
            "lockedNextValidationClass": "ground-level outfield-wall vertices excluded from this optimizer",
        },
        "fitPartition": {
            "pointControls": ["firstBase", "secondBase", "thirdBase", "homePlate"],
            "metricCircleControl": "18-foot mound circle horizontal tangencies",
        },
        "selectedPose": {
        ...
        "moundTangentFit": tangent_records,
    }
    point_pass = all(
    ...
    tangent_pass = all(
        ...
        for residual, uncertainty in zip(
            tangent_records["residualPixels"],
            tangent_records["uncertaintyPixels95"],
        )
    )
    artifact = {
        "schemaVersion": 1,
        "artifactKind": "rockies-panorama-all-regulation-field-pose-audit",
        "artifactStage": "all-passed-field-controls-refit-new-wall-holdout-pending",
        "artifactVersion": artifact_version(stable),
        **stable,
        "geometryBoundary": {
            "establishesCandidateAllFieldPose": point_pass and tangent_pass,
        ...
        "publication": {
            ...
            "blockers": [
                ...
                *([] if tangent_pass else ["MOUND_TANGENT_FIT_EXCEEDS_PIXEL_UNCERTAINTY"]),
                "ALL_FIELD_POSE_UNCERTAINTY_NOT_PROPAGATED",
