#!/usr/bin/env python3
"""Detect review candidates for numbered seat badges in nadir cube faces.

The detector finds low-saturation bright patches surrounded by saturated seat
color. It produces review diagnostics only. It does not read badge numbers,
assign rows, or establish metric controls.
"""
...
ANALYSIS_VERSION = "sportsdigita-nadir-seat-badge-candidates-v2"
...
def main() -> None:
    ...
    parser.add_argument("--maximum-area-pixels", type=float, default=1800.0)
    ...
    for section in requested:
        ...
        image_entry = next(
            (entry for entry in panorama["images"] if entry["face"] == "d"), None
        )
        if image_entry is None:
            raise ValueError(f"Section {section} has no downward cube face")
        ...
        for contour in contours:
            ...
            if width < 7 or height < 5 or width > 120 or height > 100:
                continue
            ...
            candidates.append({
                ...
                "centerPixel": [round(center[0], 3), round(center[1], 3)],
                "contourAreaPixels": round(area, 3),
                "boundingAspectRatio": round(aspect, 6),
                "meanHsv": [round(float(value), 3) for value in mean_hsv],
            })
        candidates.sort(key=lambda item: (item["centerPixel"][1], item["centerPixel"][0]))
        for index, candidate in enumerate(candidates, start=1):
            candidate["candidateId"] = f"{section}-badge-candidate-{index:03d}"
        rendered = image.copy()
        ...
        output_path = arguments.output_directory / f"section-{section}-candidates.png"
        if not cv2.imwrite(
            str(output_path), rendered, [cv2.IMWRITE_PNG_COMPRESSION, 5]
        ):
            raise ValueError(f"Could not write {output_path}")
        large_candidates = [
            candidate for candidate in candidates
            if candidate["contourAreaPixels"] >= 500.0
        ]
        tile_size = 420
        tile_columns = 4
        tile_rows = max(1, (len(large_candidates) + tile_columns - 1) // tile_columns)
        review = np.zeros((tile_rows * tile_size, tile_columns * tile_size, 3), dtype=np.uint8)
        for tile_index, candidate in enumerate(large_candidates):
            ...
            scale = min((tile_size - 40) / crop.shape[1], (tile_size - 40) / crop.shape[0])
            resized = cv2.resize(
                crop,
            ...
            review[
                origin_y + 35:origin_y + 35 + resized.shape[0],
                origin_x:origin_x + resized.shape[1],
            ] = resized
            cv2.putText(
                review,
                candidate["candidateId"],
                (origin_x + 5, origin_y + 25),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.55,
                (0, 255, 255),
                1,
        ...
        review_path = arguments.output_directory / f"section-{section}-large-candidate-review.png"
        if not cv2.imwrite(
            str(review_path), review, [cv2.IMWRITE_PNG_COMPRESSION, 5]
        ):
        ...
        section_records.append({
            ...
            "largeCandidateCount": len(large_candidates),
            "largeCandidateReviewPath": str(review_path.resolve()),
            "largeCandidateReviewSha256": sha256_file(review_path),
            "largeCandidateReviewResampling": "Lanczos diagnostic only",
        })
    stable = {
    ...
    artifact = {
        "schemaVersion": 1,
        "artifactStage": "nadir-seat-badge-review-candidates",
    ...
    print(json.dumps({
        ...
        "sections": [
            {"sectionId": record["sectionId"], "candidateCount": record["candidateCount"]}
