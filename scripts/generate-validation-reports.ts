...
import { DetailedSection, Obstruction3D } from '../src/types/stadium-complete';
...
interface StadiumValidationReport {
  stadiumId: string;
  stadiumName: string;
  city: string;
  timestamp: string;
...
function validateStadium(
  ...
  stadiumName: string,
  city: string
): StadiumValidationReport {
  const timestamp = new Date().toISOString();
  const issues: ValidationIssue[] = [];

  let data;
  try {
    data = getStadiumCompleteData(stadiumId, 'MLB');
  } catch (error) {
    return {
      stadiumId,
      stadiumName,
  ...
  const obstructionTypes: Record<string, number> = {};
  let totalVertices = 0;
  ...
  let overall: 'PASS' | 'PASS_WITH_WARNINGS' | 'FAIL';
  let summary: string;
 ...
 */
function generateMarkdownReport(report: StadiumValidationReport): string {
...
function main() {
  ...
  const outputDir = path.join(
    __dirname,
    '../.zenflow/tasks/world-cup-v2-891f/validation-reports'
  );
  ...
  const allReports: StadiumValidationReport[] = [];
 ...
 * Generate summary report
 */
function generateSummaryReport(reports: StadiumValidationReport[]): string {
...
export { validateStadium, generateMarkdownReport, main };
