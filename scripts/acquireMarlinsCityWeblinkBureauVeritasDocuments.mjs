...
const indexPath = path.resolve(option(
  'index',
  'tmp/lidar/marlins-city-weblink-bureau-veritas-2010/manifest.json',
));
const outputDirectory = path.resolve(option(
  'output-dir',
  'tmp/lidar/marlins-city-weblink-bureau-veritas-documents-2010',
));
...
if (
  index.artifactKind !== 'marlins-city-weblink-bureau-veritas-index'
  || index.source?.fileId !== '10-00847'
  || index.source?.expectedResolution !== 'R-10-0335'
) {
  throw new Error('Input is not the locked City Bureau Veritas index');
}

const selectedDocuments = index.exactInventory
...
if (selectedDocuments.length !== 8) {
  throw new Error(`Expected 8 City documents, received ${selectedDocuments.length}`);
}
...
for (const selection of selectedDocuments) {
  ...
  if (
    ...
    || documentInfo.pageInfos.filter(Boolean).length !== selection.expectedPageCount
  ) {
    throw new Error(`Unexpected City document metadata: ${JSON.stringify({ selection, documentInfo })}`);
  }
...
const stable = {
  analysisVersion: 'marlins-city-weblink-bureau-veritas-document-acquisition-v1',
  stadiumId: 'marlins',
  ...
  geometryBoundary: {
    establishesOfficialCityDocumentCopies: true,
    establishesCompleteDocumentCopyForExactFileIdAtAcquisitionTime: true,
...
const artifact = {
  schemaVersion: 1,
  artifactKind: 'marlins-city-weblink-bureau-veritas-document-acquisition',
