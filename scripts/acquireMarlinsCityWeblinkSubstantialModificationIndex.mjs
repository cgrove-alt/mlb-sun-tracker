...
const outputDirectory = path.resolve(option(
  'output-dir',
  'tmp/lidar/marlins-city-weblink-substantial-modification-2010',
));
...
const fileId = '09-00141mm';
const expectedResolution = 'R-10-0058';
...
for (const result of exactDocuments) {
  ...
  if (documentInfoResult.data.id !== result.entryId) {
    throw new Error(`Document info ID mismatch for ${result.entryId}`);
  }
  const fileName = `document-info-${result.entryId}.json`;
  const filePath = path.join(outputDirectory, fileName);
  await writeFile(filePath, documentInfoResult.bytes);
  documentInfoRecords.push({
    path: path.relative(process.cwd(), filePath),
    sha256: sha256(documentInfoResult.bytes),
    entryId: result.entryId,
    name: result.name,
    pageCount: documentInfoResult.data.pageCount,
    hasImagedPages: documentInfoResult.data.hasImagedPages,
    imagedPageCount: (documentInfoResult.data.pageInfos ?? []).filter(Boolean).length,
    repositoryPath: documentInfoResult.data.metadata?.path ?? null,
  });
}
...
const exactInventory = exactResults.map((result) => {
  ...
  return {
    ...
    publicDocumentUrl: result.type === -2
      ? new URL(
        `DocView.aspx?dbid=0&id=${result.entryId}&repo=${repositoryName}`,
        endpointBaseUrl,
      ).href
      : null,
  };
...
const candidateNamePattern = /(?:plan|drawing|section|elevation|survey|site|roof|canopy|stadium|ballpark|musp|application|exhibit)/i;
const geometryReviewCandidates = exactInventory.filter((record) => (
  record.entryType === 'document' && candidateNamePattern.test(record.name)
));
...
const stable = {
  analysisVersion: 'marlins-city-weblink-substantial-modification-index-v1',
  stadiumId: 'marlins',
  ...
  recordSummary: {
    ...
    geometryReviewCandidateCount: geometryReviewCandidates.length,
    geometryReviewCandidateEntryIds: geometryReviewCandidates.map((record) => record.entryId),
  },
  exactInventory,
  ...
  geometryBoundary: {
    ...
    establishesSupersedingConstructionAsBuiltGeometry: false,
  ...
  publication: {
    eligible: false,
    blockers: [
      'CANDIDATE_DOCUMENT_CONTENT_REVIEW_REQUIRED',
...
const artifact = {
  schemaVersion: 1,
  artifactKind: 'marlins-city-weblink-substantial-modification-index',
...
console.log(JSON.stringify({
  ...
  geometryReviewCandidates: geometryReviewCandidates.map((record) => ({
    entryId: record.entryId,
    name: record.name,
