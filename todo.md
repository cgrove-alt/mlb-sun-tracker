...
## Phase 8 — Social/visual & UX polish (DONE, verified via build + tests)

- [x] **Per-venue OG images (8.1):** app/stadium/[stadiumId]/opengraph-image.tsx generates a branded 1200x630 PNG per venue (name, team, orientation-derived shade takeaway; dome message for fixed-roof parks) via next/og. Removed the hardcoded logo512.png og:image/twitter:image from the stadium metadata so the generated image is used. Build produced **182** valid PNGs; og:image + twitter:image now point to /stadium/{id}/opengraph-image.
- [x] **Emoji-button accessibility (8.2):** the venue-page action buttons (favorite ⭐/☆, share 📤, expand ±) already had aria-label + title — verified 0 unlabeled buttons in the rendered HTML. Added aria-hidden to the decorative glyphs so screen readers announce only the label.
- [x] **At-a-glance seating-bowl SVG (8.3):** src/components/SeatingBowl.tsx — a server-rendered, color-coded bowl (Covered / back-rows Partial / shaded-side / sunny-side at 1 PM) built from real per-section baseAngle + coverage. Added to MLB venue pages (yankees renders 184 wedges: 22 covered / 109 partial / 30 shaded / 23 sun; domes render all-shaded). **Scoped:** limited to MLB, where section geometry is verified; extending to the 152 MiLB/NFL pages is a follow-up (load sections into ComprehensiveStadiumGuide + audit MiLB baseAngle quality — est. ~0.5 day).
- [x] **SSR + loading states (8.4):** verified in raw HTML that all critical guide content is server-rendered — H1, answer-first, seating bowl, sections table, 3-tier coverage cells, FAQ, related stadiums all present without JS. Interactive components (StadiumPageClient game picker, SeatRecommendationsSection) already show meaningful loading states ("Loading the {team} schedule…", LoadingSpinner).
- [x] **Fixed a Phase-5 regression:** the Rays coordinate change (→ Tropicana Field) shifted the sun-position baseline in shadeRegression.test.ts beyond ±0.05°; updated the baseline to the correct St. Petersburg coordinates. Full suite: 201/201.

### Fenway coverage accuracy (PR #76, merged, live on 7767086e85e6)

- [x] **Caching check** — bare /stadium/redsox served x-build-id=ebca87ef3eea (then 7767086e85e6 after this PR), H1=1, age 0. Auditor hit a stale cache; production was clean. No purge needed (verified my side before acting).
- [x] **FB-39..50, LB-123..136, general SRO** — were covered=true; per RateYourSeats these are NOT covered structures (covered list = PB, HPPC, EMCC/Dell, GS-1..32). Set covered=false at canonical source (also fixes the 3D rows/shade overhang) + flagged partial "day games only (press-box shade)" in the split UI. Remaining covered=true now matches RateYourSeats exactly.
