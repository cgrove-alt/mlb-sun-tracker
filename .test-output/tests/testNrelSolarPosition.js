...
const sunCalculations_1 = require("../utils/sunCalculations");
...
testLocations.forEach(location => {
    ...
    testDates.forEach(date => {
        ...
        const result = (0, nrelSolarPosition_1.computeSunPosition)(date, location.lat, location.lon, timeZoneOffset);
...
testLocations.forEach(location => {
    ...
    // NREL result
    const nrelResult = (0, nrelSolarPosition_1.getSunPositionNREL)(date, location.lat, location.lon);
    // SunCalc result
    const sunCalcPos = suncalc_1.default.getPosition(date, location.lat, location.lon);
...
console.log('\nWith NREL enabled:');
const nrelEnabledResult = (0, sunCalculations_1.getSunPosition)(new Date('2024-07-04T15:00:00'), testLocations[0].lat, testLocations[0].lon);
...
const sunCalcFallbackResult = (0, sunCalculations_1.getSunPosition)(new Date('2024-07-04T15:00:00'), testLocations[0].lat, testLocations[0].lon);
...
edgeCases.forEach(testCase => {
    console.log(`\n${testCase.name}:`);
    const result = (0, nrelSolarPosition_1.computeSunPosition)(testCase.date, testLocations[0].lat, testLocations[0].lon, -5 // EST
    );
