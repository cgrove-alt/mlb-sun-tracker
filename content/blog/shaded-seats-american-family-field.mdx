...
excerpt: "American Family Field's retractable roof eliminates sun entirely when closed, but when open, the northwest-facing orientation puts first base side in afternoon shade. Discover your best seating options."
---
...
American Family Field has a 135° orientation, giving it a distinctly southeast-facing layout. Center field points to the southeast, which means home plate faces the northwest. With this northwest-facing orientation, the first base side benefits from afternoon shade while the third base side faces into the afternoon and evening sun.
...
First base side seating is your target when the roof is open during afternoon games. The upper deck overhang provides shade to lower bowl seats along the first base line. Club level sections on the first base side benefit from structural protection and the favorable sun angle created by the northwest-facing orientation. Upper deck sections on the first base side offer direct afternoon protection from the western sun.

As the afternoon sun moves westward, the first base side's shade advantage grows. Field-level seating along the first base foul line experiences beneficial shade as the game progresses. However, prioritize open-roof planning first—if the roof is open, confirm game-time conditions and select first base seating accordingly.
...
Bring water even if the roof is closed—air-conditioned environments can be deceptive about hydration needs. If the roof is open, apply sunscreen and wear light clothing. The distinct northwest-facing orientation means weather patterns differ from surrounding cities—wind tends to flow northwest-southeast through the ballpark, which can create cooling breezes on open-roof days.

The Shadium's real-time shade tracking includes roof status information. Check the updated roof details for your specific game to confirm whether the roof will be open, then use our shade map to select first base seating if you'll be under open-air conditions.
